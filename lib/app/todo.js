...
function display_tasks(contents, proper){
	...
	lines.forEach(function(line){
		...
		if(line.charAt(0) == "x" && line.charAt(1) == " "){
			task['completed'] = true;
			line = line.substr(2);
			if(line.substr(line.indexOf(" ")).match( date_regex )){
				task['done'] = line.substr(line.indexOf(" ")).split("-");
		...
		// Rule 2
		if(line.substr(line.indexOf(" ")).match( date_regex )){
			task['date'] = line.substr(line.indexOf(" ")).split("-");
			line = line.substr(line.indexOf(" ")+1);
		}
	...
	$("<input type=text>").addClass("line").attr("placeholder", "Add a new task").appendTo("#appArea");
}
